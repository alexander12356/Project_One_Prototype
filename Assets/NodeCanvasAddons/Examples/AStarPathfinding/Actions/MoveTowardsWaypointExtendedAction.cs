...
{
	[Category("Movement")]
	[Name("Move Towards Extended Waypoint")]
	[ParadoxNotion.Design.Icon("PathfindingNode")]
	public class MoveTowardsWaypointExtendedAction : ActionTask
	{
		private const float RotationDampening = 2.0f; // To allow rotation speeds to appear same for similar values

		[RequiredField] public BBParameter<Vector3> Waypoint;

		[RequiredField] public BBParameter<float> MovementSpeed = new BBParameter<float> { value = 1.0f };
		public BBParameter<float> RotationSpeed = new BBParameter<float> { value = 1.0f };
		public BBParameter<OrientationMode> Orientation;
