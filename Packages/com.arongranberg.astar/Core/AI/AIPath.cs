...
namespace Pathfinding
{
	...
	public partial class AIPath : AIBase, IAstarAI
	{
		...
		protected virtual void UpdateMovementPlane()
		{
			...
			IMovementPlane graphTransform = graph != null
				? graph.transform
				: (orientation == OrientationMode.YAxisForward ? new GraphTransform(Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(-90, 270, 90), Vector3.one)) : GraphTransform.identityTransform);
		...
		protected override void MovementUpdateInternal(float deltaTime, out Vector3 nextPosition, out Quaternion nextRotation)
		{
			...
			if (rvoController != null)
				rvoDensityBehavior.Update(rvoController.enabled, reachedDestination, ref stopped, ref rvoController.priorityMultiplier, ref rvoController.flowFollowingStrength, currentPosition);
		...
		protected virtual void CalculateNextRotation(float slowdown, bool avoidingOtherAgents, out Quaternion nextRotation)
		{
			if (lastDeltaTime > 0.00001f && enableRotation)
			{
				...
				float rotationSpeedFactor =
					MovementUtilities.FilterRotationDirection(ref rotationFilterState, ref rotationFilterState2, lastDeltaPosition, noiseThreshold, lastDeltaTime, avoidingOtherAgents);
